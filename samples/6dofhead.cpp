...
void keyframeRelated(){
	...
	if(textureCreated){
		if(!created){
			...
			glTexImage2D(GL_TEXTURE_2D, 0, 3, glTexture.sizeX, glTexture.sizeY, 0, GL_BGR, GL_UNSIGNED_BYTE, glTexture.data);
...
void generateTexture(){
	...
	glReadPixels(0,0,640,480,GL_BGR, GL_UNSIGNED_BYTE, data);
	if(!generatedImage){
		generatedImage = cvCreateImage( cvSize( 640, 480 ),
				IPL_DEPTH_8U, 3);
	}
	...
	for( int y=0; y<480; y++ ) {
	  uchar* ptr = (uchar*) (
	     generatedImage->imageData + y * generatedImage->widthStep
	  );
