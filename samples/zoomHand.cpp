...
unsigned char jpegData[10000000];
...
int initialGuess=1;
int drawImage=1,drawAxis=0,drawNormals=0,blending=0,drawLight=1,drawBackground=1;
...
void getJPEGAsGLTexture(const char* imageFile, GLuint* glTex){		
	
	IplImage* currentFrame = cvLoadImage(imageFile);

    // texture 1 (poor quality scaling)
    glBindTexture(GL_TEXTURE_2D, *glTex);   // 2d texture (x and y size)
...
void drawJpegImage(){
	...
	glBegin(GL_QUADS);		                
			    ...
			    glVertex3f(-1.0f*scale, -1.0*scale,  0.0f);	// Bottom Left Of The Texture and Quad
			    
			    
			    glTexCoord2f(1.0f, 1.0f);
			    glVertex3f( 1.0f*scale, -1.0*scale,  0.0f);	// Bottom Right Of The Texture and Quad
			    
			    
			    
			    
			    glTexCoord2f(1.0f, 0.0f);
			    glVertex3f( 1.0f*scale,  1.0f*scale, 0.0f);	// Top Right Of The Texture and Quad
			    		    
			    
			    glTexCoord2f(0.0f, 0.0f);
			    glVertex3f( -1.0f*scale,  1.0f*scale,  0.0f);
		glEnd();
...
void getFrameAsGLTexture(){	
	...
	IplImage* currentFrame = cvLoadImage("data//browser.jpg");
...
void drawGrabbedFrame(){
	...
	glBindTexture(GL_TEXTURE_2D, imageTexture);   // choose the texture to use.
...
{  
	...
	if(initialGuess){		
		...
		glPositMatrix[0]=1.0f;
		glPositMatrix[4]=0.0f;
		glPositMatrix[8]=0.0f;
		
		glPositMatrix[1]=0.0f;
		glPositMatrix[5]=-1.0f;
		glPositMatrix[9]=0.0f;
		
		glPositMatrix[2]=0.0f;
		glPositMatrix[6]=0.0f;
		glPositMatrix[10]=-1.0f;
		
		//glPositMatrix[8]=0.0f;
		//glPositMatrix[9]=0.0f;
		//glPositMatrix[10]=0.0f;
		//glPositMatrix[11]=0.0f;
...
void InitGL(GLsizei Width, GLsizei Height)	// We call this right after our OpenGL window is created.
{
    ...
    getJPEGAsGLTexture("data//browser.jpg",&frameTexture);
    getJPEGAsGLTexture("data//soc08.jpg",&imageTexture);
...
void openGLCustomInit(int argc, char** argv ){
	...
	glutInitWindowSize(1680/2, 1050/2);  

	/* the window starts at the upper left corner of the screen */
	glutInitWindowPosition(0, 0);  

	/* Open a window */  
	window = glutCreateWindow("EHCI (Zoom and Rotate) - http://code.google.com/p/ehci");
